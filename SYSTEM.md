...
**오늘 밤(8/23 22시) 첫 실전 검증 대기 중**: JTBC 뉴스룸 전체 수집(「더보기」 수정, 기대 20건대) · KBS부산 뉴스9 클로징 폴링(기대 8~10건) · 실패 매체 자동 보충(nightly-retry) · 지면 상한 60 · 부산MBC 2페이지. 다음 세션은 8/23 밤 nightly 로그(`gh run list --workflow=nightly.yml`)에서 이 숫자부터 확인할 것.
...
- **건수 상한 점검 기준**: 매일 같은 숫자(JTBC 11, 지면 40/40)는 상한 신호. 단 KNN 뉴스아이 11건은 실제 방송 전체(날씨 코너로 끝남, 페이지 없음)라 정상. 지면 상한은 60, 부산MBC는 1페이지 마지막 행이 대상일이면 2페이지(`?page=2&mt=A&subt=4&smat=A&sl=`)까지 읽는다.

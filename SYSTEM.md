...
4. **사안 중복 억제**(2026-08-13): 12시간 내 전송분과 제목 토큰 겹침 ≥0.7(`DUP_OVERLAP`)이면 재탕으로 기록만. 숫자 토큰 제외·토큰 집합화(여론조사 수치·단어 반복 오탐 방지). 단독·속보는 예외. 날씨 안내는 하루 1건(`state.wxDate`, 재해성 제목 예외). 시당 방은 24h `chiefRecent`로 추가 차단. 하루 ~125건(15~21%) 억제.

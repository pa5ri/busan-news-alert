...
| 시각(KST) | 동작 |
|---|---|
| 상시 2분 | 실시간 수집·발송, **급증 감지(사안 단위)**: 1시간 기사를 토큰 겹침 0.5로 묶어 8건↑ && 평소 하루치 절반↑이면 알림, 같은 사안은 2배로 불 때만 「확산 갱신」, 주기당 1건. 메시지에 해석 블록(평소 대비 배수·매체군 구성·부산 관련성·이슈 대장 연결·기준). **단독 확산 추적**: [단독] 뒤 6h 내 주요 매체 3곳↑ 후속이면 알림. 상태: `state.surged`(당일)·`state.scoopTrack`·`state.recentSent`(1h) |
| 55분 주기 | 사설 체크 |

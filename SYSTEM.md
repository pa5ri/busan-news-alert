...
## 0. 현재 상태 스냅샷 (2026-08-23, 세션 인계용)

**9/18 정기 점검 결과**: JTBC 29~34건·KBS부산 9~11건 정상 확인(8/23 검증 완료). 부산MBC 뉴스데스크가 9/13부터 22시 보고에서 매일 0건 → HTML 직접 파싱 이중 경로 추가(9/18 밤 첫 적용, 로그의 `부산MBC 진단`·`HTML 직접 파싱` 줄 확인). 지면 상한 150. **사용자 결정(9/18)**: 본문에만 부산이 스친 기사(분야방 전송의 약 27%)도 지금처럼 분야방 전송 유지 — 참고방 분리·전송 생략 모두 기각.

**사용자가 확정한 운영 원칙(바꾸지 말 것)**
- 완결성 > 즉시성. 보고가 늦어도 빠짐없이.
- **컴퓨터 없이 클라우드만으로** 동작해야 한다. 로컬 PC 경로(의정 체크, JTBC 21:40 수집)는 보조일 뿐.
- 아카이브는 전량 유지(인사이트용), 줄이는 건 방 전송뿐.
- 해석(인사이트) 블록은 급증·확산 갱신·단독 확산 알림에만. 개별 단독·속보엔 안 붙임.
- 모집단 = 부산 검색 수집분. 전재수 언급은 그 안에서 세되 누락 금지(색인으로 꼬리표).
- 봇 표시명 5개 모두 「부산 통합 모니터링 봇」. 연속 메시지에서 이름이 생략되는 건 텔레그램 규칙 — 그대로 둠.
- 전용 방 체계(분야방 대신 단독 발송): 기고·칼럼 → 시의회(+조례) → 시당위원장 → 중요시책 → 사건사고/날씨재난. 인터뷰·르포만 분야방과 병행.

**보류·대기(사용자 결정 필요)**
- C1: 제목에 부산 없음 + 군소 매체라 모집단에서 빠지는 전재수 기사(주 61건) — 모집단에 들일지 보류.
- 2층 AI 요약($5 충전+API 키), 3층 등급 버튼, 단독·속보 전수 수집 — 미착수.
...
- 텔레그램 포럼 그룹 「부산 통합 모니터링」(chat_id `-1004297621610`) 하나에 주제(방) 26개로 전부 배달.
...
{"정치":4,"경제":5,"사회":6,"생활/문화":7,"IT/과학":8,"세계":9,"스포츠":10,
 ...
 "사건사고":25676,"날씨재난":25678,"중요시책":27111,
 "중앙여론조사":41385,"부산여론조사":41389}
```
...
3. 분야방 발송 후 추가 발송: 단독·속보방, 인터뷰/르포방, 시당위원장방(맥락 단락 `…ctx…` 포함 동일 형식). **분야방 '대신' 전용 방에만 가는 예외**(아카이브 분야 태그는 유지): 시의회(8/23) · 기고·칼럼(8/24) · **사회 세분화(8/26): 사회 기사 중 사건·사고(socialSub, 🚨)와 날씨·재난(🌦)은 각 방으로만** — 사회방 285건/일 중 사건 70·재난 58건이 분리, 사회방엔 ~155건/일 남음 · **중요시책(8/29 신설→9/1 개편, 🎯 isAgenda)**: 돔구장·글로벌허브도시(특별법)·산은/공공기관 이전·북극항로·가덕신공항·북항 재개발/복합환승센터·해수부 부산 이전/신청사·민생100일 제목은 **중요시책방에만**(~13건/일, 비유 워딩에도 우선 적용) · **시당위원장(8/30)**: 박홍배·이성권·시당 조직명 기사는 시당방에만(분야방 병행 폐지). **여론조사(9/19 신설, pollKind — 제목 기준)**: 📊 중앙 여론조사(갤럽·NBS·리얼미터 등 국정·정당, 메이저만, 같은 대표 수치 2건·기관별 3건/일 상한, 부산 지역지는 상한 없음) · 📍 부산 여론조사(제목에 부산·전재수·부울경·중요시책어, 또는 시도지사 직무평가 기사 본문에 전재수 순위 — 매체 불문). 타 시도지사·교육감 평가(부산 수치 없음)는 대상 아님. **지수·평판(9/19 추가, indexKind, 라벨 [부산 지수·평판])**: K-브랜드지수(광역단체장=전재수 순위·부산 구청장/군수 부문), 도시 브랜드평판(부산시 순위), 국제 도시지수(GFCI 등)도 부산 여론조사 방으로 — 발표당 3건(제목에 부산·전재수 없는 간접 기사는 2건), 은행·공공기관·CEO 평판은 제외. 본 패스 외에 **여론조사 전용 검색 패스(runPollPass, 10분 주기, 질의 6개)**가 본문에 부산이 없는 정례 조사를 보충(방에만 발송, 아카이브 미포함). 중복은 pollSeen/pollTitles/pollRecent. **라우팅 우선순위(9/1 확정, 9/19 여론조사 추가): 기고·칼럼(형식) → 여론조사(형식) → 시의회 → 시당위원장 → 중요시책 → 사회 세분화(사건사고/날씨재난) → 분야방.** (시의회·시당 기사는 시책어가 있어도 원래 방으로 — 사용자 결정)
4. **사안 중복 억제**(2026-08-13): 12시간 내 전송분과 제목 토큰 겹침 ≥0.7(`DUP_OVERLAP`)이면 재탕으로 기록만. 숫자 토큰 제외·토큰 집합화(여론조사 수치·단어 반복 오탐 방지). 단독·속보는 예외. 날씨 안내는 하루 1건(`state.wxDate`, 재해성 제목 예외). **의례성 기사**(isCeremonial: 포토 캡션·운세·오늘의 일정·부고/인사/동정, ~5건/일)는 상시 기록만. 시당 방은 24h `chiefRecent`로 추가 차단. 합계 하루 ~130건(15~21%) 억제. **2026-09-18 가중 판정 추가**: 이틀치 아카이브 제목 IDF로 가중 겹침 ≥0.5(`DUP_WEIGHTED`)+공통 토큰 2개↑면 재탕(같은 보도자료 매체별 버전 추가 ~15% 억제). 단독·속보(0.8)는 단순 판정만. **원칙: 아카이브는 전량 유지(인사이트·브리핑·대장용), 줄이는 건 방 전송뿐.**
5. 전송은 3.5초 페이싱 + 429 재시도 4회 + **성공 후에만 seen 마킹**(실패분 자동 이월).
6. 아카이브는 전송 여부와 무관하게 전량 기록.

특수 유형 판별 요약:
- 인터뷰 = 제목에 `인터뷰` AND `전재수|부산시장`
- 르포/기고 = 제목 `[…르포…]` / `[…(기고|칼럼|시론|기자수첩|취재수첩|기자의 눈|데스크|세상읽기|논단|발언대|시평|오피니언)…]` (괄호 안 글자수 제한 없음, 제목 뒤 괄호도 인정, [사설] 제외) AND (부산지역지 OR isBusanRelevant OR **제목에 타 광역지명 없음**) ← 2026-08-12 확정 원칙, 2026-08-24 칼럼류 확장. **기고·칼럼은 분야방 대신 기고방에만**(시의회와 동일), 르포·인터뷰는 병행.
...
- 시의회 = 제목에 `부산시의회|부산광역시의회|부산광역시 시의회|부산 시의회|부산시의원|부산 시의원` (councilNews, 본문 매칭 제외 — 실측상 본문 전용 매치는 무관 기사 다수). **+ 조례 기사**(제목에 조례 + 부산 관련, 타 지역 제목 제외)도 시의회방으로 — 라벨 [조례 동향] (2026-08-31)
...
- **KBS부산 뉴스9는 21:40~22:20에 방송되며 리포트가 방송 중 순차 게시**된다 — 22:01에 긁으면 3~6건(마커 없음). 대상일이 오늘이면 **「뉴스9 부산 클로징」 마커가 게시될 때까지 2분마다 재확인**(최대 22:40), 순서는 맨 뒤(2026-08-23). 보고 도착은 22:15~22:45 사이. **원칙: 완결성 > 즉시성, PC 없이 클라우드만으로**(사용자 결정).
- **건수 상한 점검 기준**: 매일 같은 숫자(JTBC 11, 지면 40/40)는 상한 신호. 단 KNN 뉴스아이 11건은 실제 방송 전체(날씨 코너로 끝남, 페이지 없음)라 정상. 지면 상한은 150(60일 때 국제신문이 매일 60에 걸림, 2026-09-18), 부산MBC는 1페이지 마지막 행이 대상일이면 2페이지(`?page=2&mt=A&subt=4&smat=A&sl=`)까지 읽는다.

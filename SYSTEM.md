...
**9/18 정기 점검 결과**: JTBC 29~34건·KBS부산 9~11건 정상 확인(8/23 검증 완료). 부산MBC 뉴스데스크가 9/13부터 22시 보고에서 매일 0건 → HTML 직접 파싱 이중 경로 추가(9/18 밤 첫 적용, 로그의 `부산MBC 진단`·`HTML 직접 파싱` 줄 확인). 지면 상한 150. **사용자 결정 대기**: 본문에만 부산이 스친 기사(분야방 전송의 약 27%) 처리 방안.
